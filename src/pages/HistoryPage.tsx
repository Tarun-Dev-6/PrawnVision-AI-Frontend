...
import {
  getUserLocalCaptures,
  deleteUserLocalCapture,
  LocalCapture,
} from "@/services/localCaptures";
...
export const HistoryPage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const data = getUserLocalCaptures(user.id);
      setCaptures(data);
      ...
      for (const c of data) {
        try {
          ...
          console.error("Image resolve failed:", c.imagePath);
        }
      }
  ...
  const handleDelete = (id: number) => {
    ...
    deleteUserLocalCapture(user.id, id);
    setCaptures((prev) => prev.filter((c) => c.id !== id));

    toast({
      title: "Deleted",
      description: "Capture removed from history",
    });
  };
              ...
              >
                ...
                >
                  <img
                    ...
                    className="w-full h-full object-cover"
                    onError={() =>
                      console.error("FAILED IMAGE:", imageUris[c.id])
                    }
                  />
